...
public class RSPvtActivity extends AppCompatActivity {

    private SQLiteDatabase db;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_rspvt);
        InputStream inputStream = getResources().openRawResource(R.raw.rspvtbill);
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();

        db = openOrCreateDatabase("system", MODE_PRIVATE, null);
        db.execSQL("create table if not exists rspvtbill (year string,title varchar(30),name varchar(50),ministry varchar(20),category varchar(30),primary key(year,title),foreign key(name) references rsmember(name));");
        ...
        for(int j=0;j<100;j++)
        {
            ...
            db.insert("rspvtbill",null,cv);
        }



        Cursor cursor=db.rawQuery("select * from rspvtbill",null);
